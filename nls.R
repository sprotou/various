...
result_dataframe <- data.frame(matrix(numeric(),ncol = 10, nrow = nrow(variables_set)))
colnames(result_dataframe) <- c("model","variable0","variable1","variable2","Parameter Estimates","std","t-values","AIC","AICC","BIC")
for(i in 1 : nrow(variables_set)){
    model_num <- paste("model",i,sep = "") 
    ...
    nlsmean=function(b, zPriceVar, var1,var2) { #added parameters to input any variable for advertisement type and price
        mu <- b[1]+b[2]/(zPriceVar)^b[3] + b[4]*(var1)^b[5] + b[6]*(var2)^b[7]		# mu denotes the mean function
        return(mu)
    }
    ...
    result_dataframe[i,"Parameter Estimates"] 
    result_dataframe[i,"std"] 
    result_dataframe[i,"t-values"] 
    result_dataframe[i,"AIC"] <- aic
    result_dataframe[i,"AICC"] <- aicc
    result_dataframe[i,"BIC"] <- bic 
}
result_dataframe
# x <- data_div_sd$Radio
